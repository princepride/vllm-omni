...
def _make_state(request_id: str, branch, video_rows, audio_rows, sigmas_video, sigmas_audio):
...
def _step_pipeline(model, *, packed_batch_supported: bool = True):
    ...
    pipeline._packed_batch_supported = lambda: packed_batch_supported
    return pipeline
...
def test_single_request_batch_layout_matches_request_mode():
    ...
    for key in ("x", "audio_x", "img_position_ids", "update_mask", "token_tags", "prompt_embeds"):
        torch.testing.assert_close(batched_kwargs[key], request_kwargs[key])
    for key in ("img_pos_info", "audio_pos_info", "text_pos_info", "img_pos_for_infer_output_info"):
        torch.testing.assert_close(batched_kwargs[key]["position_ids"], request_kwargs[key]["position_ids"])
    # Same per-row timesteps, reconstructed through the unique/inverse pair.
    torch.testing.assert_close(
        batched_kwargs["unique_timesteps"][batched_kwargs["inverse_indices"]],
        request_kwargs["unique_timesteps"][request_kwargs["inverse_indices"]],
    )
    torch.testing.assert_close(
        batched_kwargs["packed_seq_params"]["cu_seqlens_q"],
        request_kwargs["packed_seq_params"]["cu_seqlens_q"],
    )
    # The batched builder reports the true longest document. Request mode reports
    # only the real one, which is the same value for any production layout (the
    # padding tail is under 64 rows while the real document is far longer).
    bounds = batched_kwargs["packed_seq_params"]["cu_seqlens_q"].tolist()
    longest = max(stop - start for start, stop in zip(bounds[:-1], bounds[1:]))
    assert batched_kwargs["packed_seq_params"]["max_seqlen_q"] == longest
    assert batched_kwargs["packed_seq_params"]["max_seqlen_q"] >= request_kwargs["packed_seq_params"]["max_seqlen_q"]
...
def test_batched_forward_matches_per_request_forwards():
    ...
    from vllm_omni.diffusion.models.minimax_h3.step_batch import (
    ...
    video_parts = minimax_h3_split_rows(
        batched_video, [int(branch.img_pos.shape[0]) for branch in branches], field_name="video"
    )
    audio_parts = minimax_h3_split_rows(
        batched_audio, [int(branch.audio_pos.shape[0]) for branch in branches], field_name="audio"
    )
...
def test_step_execution_matches_request_mode_denoise_loop(packed_batch_supported):
    ...
    state = _make_state("req-0", branch, video_rows, audio_rows, sigmas_video, sigmas_audio)
...
def test_batched_step_execution_matches_independent_requests():
    ...
    for spec, (sigmas_video, sigmas_audio) in zip(specs, schedules):
        ...
        state = _make_state("solo", branch, video_rows, audio_rows, sigmas_video, sigmas_audio)
        while not state.denoise_completed:
    ...
    for index, (spec, (sigmas_video, sigmas_audio)) in enumerate(zip(specs, schedules)):
        branch, video_rows, audio_rows = _make_branch(**spec)
        states.append(_make_state(f"req-{index}", branch, video_rows, audio_rows, sigmas_video, sigmas_audio))
...
def test_cu_seqlens_length_signals_a_multi_request_layout():
    """Attention detects packed batches by cu_seqlens length, so pin that down.

    ``MiniMaxH3Attention._run_packed_attention`` skips both the KV prefix
    length and the 1-D pad mask when ``cu_seqlens.numel() > 3``, because neither
    can describe a block-diagonal batch. That check only stays correct while a
    single request keeps producing at most three bounds.
    """
    from vllm_omni.diffusion.models.minimax_h3.step_batch import minimax_h3_batched_forward_kwargs

    def bounds(branches, videos, audios):
    ...
    solo = bounds([first], [first_video], [first_audio])
    assert solo.numel() <= 3, "a single request must stay on the prefix-mask path"

    batched = bounds([first, second], [first_video, second_video], [first_audio, second_audio])
    assert batched.numel() > 3, "a packed batch must be distinguishable from a single request"
