...
def test_fl2va_refine_prepares_from_disaggregated_encoder_output(pipeline):
    ...
    conditioning = MiniMaxH3EncoderConditioning(
        ...
        visual_condition=torch.ones(64, 96),
        visual_condition_shapes=((1, 16, 16),),
        keyframe_frame_indices=(0,),
    )
    prompt = {"additional_information": {"encoder_output": conditioning.to_omni_payload()}}
    sampling = OmniDiffusionSamplingParams(
        height=256,
        width=256,
        num_frames=num_frames,
        num_inference_steps=2,
        extra_args={"task": "fl2va", "latent_upscale": 2.0, "latent_refine": 0.5},
    )

    context = pipeline._prepare_request_inputs(prompt, sampling)

    assert (context["latent_upscale"].latent_height, context["latent_upscale"].latent_width) == (32, 32)
    assert context["latent_refine"].strength == 0.5
    assert context["keyframe_frame_indices"] == [0]
    assert context["visual_condition_shapes"] == [(1, 16, 16)]
...
def test_mixed_reference_conditioning_matches_stage_zero_handoff(pipeline, monkeypatch):
    ...
    from vllm_omni.model_executor.stage_input_processors.minimax_h3 import encoder2diffusion, prepare_encoder_prompt
    ...
    source = SimpleNamespace(finished=True, outputs=[SimpleNamespace(multimodal_output=flatten_payload(payload))])
