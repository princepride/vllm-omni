...
class GPUWorker:
    ...
    def _init_omni_connector(self) -> None:
        ...
        try:
            ...
            # 1. Try to get from omni_config (injected from YAML)
            # Use self.od_config because self.vllm_config is a dummy VllmConfig without model_config
            if self.od_config.omni_config:
                connector_config = self.od_config.omni_config.get("connector_config")
    ...
    def _receive_kv_cache_for_request(self, req: OmniDiffusionRequest) -> None:
        ...
        try:
            ...
            omni_config = self.od_config.omni_config
            stage_id = omni_config.get("stage_id")
            engine_input_source = omni_config.get("engine_input_source", [])
