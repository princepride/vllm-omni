...
class MiniMaxH3Pipeline(
    ...
    def _refine_keyframe_condition(
        ...
        """Resize the encoder-stage FL2VA keyframe latents once per request."""
        cached = context.get(_REFINE_KEYFRAME_CONDITION)
        if cached is None:
            target = context["latent_upscale"]
            shapes = context["visual_condition_shapes"] or []
            condition = context["visual_condition"]
            if condition is None or not shapes:
                raise OmniClientError("FL2VA latent_refine requires encoder keyframe conditions")
            if any(shape != shapes[0] for shape in shapes) or shapes[0][0] != 1:
                raise OmniClientError("FL2VA latent_refine requires equally sized keyframe conditions")
            _, height, width = shapes[0]
            rows_per_keyframe = (height // 2) * (width // 2)
            if int(condition.shape[0]) != len(shapes) * rows_per_keyframe:
                raise OmniClientError("FL2VA latent_refine keyframe rows do not match their shapes")
            keyframes = minimax_h3_unpatchify_video_tokens(
                condition,
                latent_shape=(1, height // 2, width // 2, 24),
                patch_size=(1, 2, 2),
            )
            upscaled = self._upscaled_latent(keyframes, target)
            cached = (
                minimax_h3_patchify_video_latent(upscaled, patch_size=(1, 2, 2)),
                [(1, target.latent_height, target.latent_width)] * len(shapes),
            )
