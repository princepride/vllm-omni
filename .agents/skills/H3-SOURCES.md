...
- `h3-prompt-writing/references/base-en.txt` and `ref-en.txt` are copied unchanged
  from the source snapshot. The entrypoint adds a portable execution reference
  and clarifies clip/window duration for verified long-video backends.
...
the pinned snapshot. This provenance record preserves attribution and does not
assert a new license for upstream material or assume that this repository's code
license relicenses it. Existing source text and attribution should travel with
any redistribution; check upstream terms when publishing a separate package.
